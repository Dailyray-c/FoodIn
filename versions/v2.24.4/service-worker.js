const CACHE_NAME = 'food-inventory-v92';   // v2.24.4：①★根治 API 缓存污染 —— fetch 改白名单模式，只拦同域静态资源+CDN，条码查询(barcode)/OFF 等第三方 API 一律放行走网络（旧黑名单漏了 apizero.cn，其响应被永久缓存，导致批量页同一条码第二次起永远返回旧结果、不再真发请求）；②合并 2.24.3 云同步致命修复；③设置页 12 张 section-card 图标容器补 flex-shrink-0。缓存号递增 → 旧 SW 失效，activate 时删除旧缓存（连带清除设备上的污染 API 响应）
